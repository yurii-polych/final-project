...
class WeatherService:
    ...
    which are the URLs for the geocoding and weather APIs respectively. It has one static method, get_geo_data, which takes 
    a city name as input and returns the current weather data for that city. It first sends a request to the geocoding API 
    to get the latitude and longitude of the city, and then sends a request to the weather API to get the current weather data 
    for that location. If the request to the geocoding API fails, it raises a WeatherServiceException. 
    If the request to the weather API succeeds, it returns a string containing the current temperature, wind speed, and wind direction for
    """
    ...
    def get_current_weather_by_geo_data(lat, lon):
        ...
        @return a string containing the current weather data for the given location. The string contains the temperature, wind speed, and wind direction.
        """
        params = {
            'latitude': lat,
            'longitude': lon,
